...
# ── Thresholds ────────────────────────────────────────────────────────
BER_THRESHOLD = 0.05   # BER > 0.05  → tampered
NC_THRESHOLD  = 0.97   # NC  < 0.97  → tampered
...
# ── Build a speaker-aware registration key from an uploaded filename ───
# TIMIT files arrive as  DR2_FDRD1_SA1.WAV  or  SA1.WAV  etc.
# We strip attack suffixes first, then use whatever remains as the key.
def build_reg_key(filename):
    """Return the registration key (no extension, no attack suffix)."""
    name = os.path.splitext(filename)[0]
    for suffix in ATTACK_SUFFIXES:
        if name.lower().endswith(suffix):
            name = name[:len(name) - len(suffix)]
            break
    return name                            # e.g. "DR2_FDRD1_SA1"

def find_matching_pkl(filename):
    key      = build_reg_key(filename)
    pkl_name = f"registered_{key}.pkl"
    exact    = os.path.join(OUTPUT_PATH, pkl_name)
    if os.path.exists(exact):
        return exact, pkl_name
    # case-insensitive fallback
    for f in os.listdir(OUTPUT_PATH):
        if f.lower() == pkl_name.lower():
...
def do_register(audio_path, original_filename=None):
    # Use the original upload filename for the key so speaker info is preserved
    src_name = original_filename or os.path.basename(audio_path)
    key_name = build_reg_key(src_name)
    pkl_path = os.path.join(OUTPUT_PATH, f"registered_{key_name}.pkl")

    y, _     = librosa.load(audio_path, sr=SR, mono=True)
    feats    = extract_features(y)
    thresh   = np.mean(feats, axis=0)
    binary   = (feats >= thresh).astype(np.uint8).flatten()
    henon_k  = henon_key(len(binary))
    enc      = np.bitwise_xor(binary, henon_k)
    data = {
    ...
    return {
        ...
        "n_bits"        : len(binary),
        "key_preview"   : list(map(int, henon_k[:20])),
...
def do_verify(audio_path, pkl_path):
    ...
    henon_k     = henon_key(len(binary_q), reg["henon_x0"], reg["henon_y0"],
                            reg["henon_a"], reg["henon_b"])
    dec_reg     = np.bitwise_xor(reg["encrypted"][:len(binary_q)], henon_k[:len(binary_q)])
    ber         = compute_ber(binary_q, dec_reg)
    nc          = compute_nc(binary_q, dec_reg)
    fp_tampered = (ber > BER_THRESHOLD) or (nc < NC_THRESHOLD)

    # ── Final decision ─────────────────────────────────────────────────
    tampered = rf_tampered or fp_tampered
    ...
    return {
        ...
        "rf_tampered"   : rf_tampered,
        "fp_tampered"   : fp_tampered,
        "rf_label"      : "TAMPERED" if rf_tampered  else "AUTHENTIC",
...
def api_register():
    ...
    f.save(path)
    try:
        return jsonify(do_register(path, original_filename=f.filename))
...
def api_match_pkl():
    ...
    if pkl_name:
        ...
        base = build_reg_key(filename)
        return jsonify({"found": False, "base_name": base})
...
def api_verify():
    ...
    if not pkl_path:
        base = build_reg_key(f.filename)
        return jsonify({
            "error"    : f"No registered watermark found for '{base}'.",
            "no_match" : True,
            "base_name": base,
        }), 404
    try:
